<!-- ========================= HEADER ========================= -->

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=8A2BE2&height=230&section=header&text=Walter%20Prestes&fontSize=45&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Java%20Developer%20•%20Flutter%20Developer%20•%20Future%20Full%20Stack&descAlignY=60" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=A855F7&center=true&vCenter=true&width=850&lines=👋+Eai,+eu+sou+Walter+Prestes;☕+Java+Developer;📱+Flutter+Mobile+Developer;🤖+Python+Automation+Student;🚀+Sempre+aprendendo+novas+tecnologias" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Java-8A2BE2?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring%20Boot-8A2BE2?style=for-the-badge&logo=springboot&logoColor=white"/>
  <img src="https://img.shields.io/badge/Flutter-8A2BE2?style=for-the-badge&logo=flutter&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-8A2BE2?style=for-the-badge&logo=python&logoColor=white"/>
</p>

---

# 👨‍💻 Sobre Mim

<p align="center">
Apaixonado por tecnologia, desenvolvimento de software e automação.<br>
Atualmente focado em <strong>Java</strong>, <strong>Spring Boot</strong>, arquitetura <strong>MVC</strong>,
desenvolvimento mobile com <strong>Flutter</strong> e automações com <strong>Python</strong>.
</p>

```java
public class WalterPrestes {

    String nome = "Walter Prestes";

    int idade = 17;

    String escolaridade =
        "3º Ano do Ensino Médio + Desenvolvimento de Sistemas (SENAI)";

    String[] habilidades = {

        "Java",
        "Spring Boot",
        "MVC",
        "Flutter",
        "HTML",
        "CSS",
        "JavaScript",
        "MySQL",
        "Git",
        "GitHub",
        "Python"
    };

    String objetivo =
        "Me tornar um Desenvolvedor Full Stack e Mobile de alto nível.";
}
---

# 🛤️ Minha Jornada

```txt
📅 2025
└── 🚀 Início do curso Técnico em Desenvolvimento de Sistemas (SENAI)
    ├── Banco de Dados
    ├── Lógica de Programação
    ├── HTML & CSS
    ├── Java
    └── MySQL

📅 2026
└── 📚 Aprofundamento em Java
    ├── Arquitetura MVC
    ├── Spring Boot
    ├── JavaScript
    ├── Flutter
    ├── APIs REST
    ├── Python
    └── Git & GitHub

📅 Futuro
└── 🎯
    ├── Conseguir meu primeiro estágio
    ├── Me tornar Desenvolvedor Full Stack
    ├── Especialização em Mobile
    ├── Cloud Computing
    └── Inteligência Artificial
```

---

# 🚀 Projetos em Destaque

## ☕

### Sistema de Gestão em Java MVC

<p align="center">
<img src="COLOQUE_AQUI_A_IMAGEM_DO_PROJETO" width="800">
</p>

Sistema desktop desenvolvido utilizando a arquitetura MVC.

### Tecnologias utilizadas

- ☕ Java
- 🌱 Spring Boot
- 🏗️ MVC
- 🗄️ MySQL

<p align="center">

<a href="LINK_DO_REPOSITORIO">
<img src="https://img.shields.io/badge/💻%20Código-8A2BE2?style=for-the-badge">
</a>

<a href="LINK_DO_PROJETO">
<img src="https://img.shields.io/badge/🚀%20Projeto-8A2BE2?style=for-the-badge">
</a>

</p>

---

## 📱

### Aplicativo Mobile em Flutter

<p align="center">
<img src="COLOQUE_AQUI_A_IMAGEM_DO_APP" width="800">
</p>

Aplicativo desenvolvido utilizando Flutter com interface moderna e responsiva.

### Tecnologias utilizadas

- 📱 Flutter
- 🎯 Dart
- 🔥 Firebase

<p align="center">

<a href="LINK_DO_REPOSITORIO">
<img src="https://img.shields.io/badge/💻%20Código-8A2BE2?style=for-the-badge">
</a>

<a href="LINK_DO_PROJETO">
<img src="https://img.shields.io/badge/📱%20Aplicativo-8A2BE2?style=for-the-badge">
</a>

</p>

---

## 🌐

### Sites desenvolvidos por mim

Projetos criados utilizando HTML, CSS e JavaScript.

✔ Landing Pages

✔ Sites Institucionais

✔ Portfólios

✔ Interfaces Responsivas

### Tecnologias

<p align="center">

<img src="https://skillicons.dev/icons?i=html,css,js"/>

</p>

---

# 🏆 Conquistas

🏅 Desenvolvedor Java

🏅 Desenvolvimento Mobile com Flutter

🏅 Arquitetura MVC

🏅 Desenvolvimento Web

🏅 Banco de Dados MySQL

🏅 Git & GitHub

🏅 Aprendizado contínuo

---

# 🌎 Idiomas

🇧🇷 Português
⭐⭐⭐⭐⭐ Nativo

🇺🇸 Inglês
⭐⭐⭐☆☆ Básico / Intermediário

---

# 📊 GitHub Stats

<p align="center">

<img
height="180em"
src="https://github-readme-stats.vercel.app/api?username=walterprestes007-bot&show_icons=true&theme=github_dark&hide_border=true&title_color=A855F7&icon_color=A855F7&text_color=ffffff"/>

<img
height="180em"
src="https://github-readme-stats.vercel.app/api/top-langs/?username=walterprestes007-bot&layout=compact&hide_border=true&theme=github_dark&title_color=A855F7&text_color=ffffff"/>

</p>

---

# 🔥 GitHub Streak

<p align="center">

<img src="https://streak-stats.demolab.com?user=walterprestes007-bot&theme=github-dark&hide_border=true&ring=A855F7&fire=A855F7&currStreakLabel=A855F7"/>

</p>

---

# 📈 Activity Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=walterprestes007-bot&theme=github-compact&hide_border=true&color=A855F7&line=A855F7&point=ffffff"/>

</p>

---

# 🐍 Contribuições

<p align="center">

<img src="https://raw.githubusercontent.com/walterprestes007-bot/walterprestes007-bot/output/github-contribution-grid-snake.svg"/>

</p>

---

# 🎧 Tocando Agora

<p align="center">

<a href="https://open.spotify.com/intl-pt/">

<img src="https://novatorem.vercel.app/api/spotify"/>

</a>

</p>

> ⚠️ **Observação:** Para esse card funcionar é necessário configurar o **Novatorem** com sua conta do Spotify.

---

# 💻 Meu Setup

<div align="center">

| Hardware | Software |
|:---------:|:---------:|
| 💻 Desktop | ⚡ VS Code |
| 🖥️ Windows | ☕ Java |
| 📱 Android | 📱 Flutter |
| 🌐 GitHub | 🌱 Spring Boot |
| 🗄️ MySQL | 🤖 Python |

</div>

---

# 📅 Rotina de Estudos

```text
☀️ Estudo Java

☀️ Spring Boot

🌇 Desenvolvimento Web

🌙 Flutter

🌙 Projetos pessoais

🌙 GitHub

🌙 Aprender algo novo todos os dias 🚀
```

---

# 🎯 Metas para 2026

✅ Conseguir meu primeiro estágio

✅ Construir um portfólio profissional

✅ Dominar Java

✅ Dominar Spring Boot

✅ Evoluir em Flutter

✅ Criar aplicações completas Full Stack

✅ Publicar aplicativos Mobile

✅ Aprender Cloud Computing

✅ Aprender Inteligência Artificial

---

# 🏆 GitHub Trophies

<p align="center">

<img src="https://github-profile-trophy.vercel.app/?username=walterprestes007-bot&theme=matrix&no-frame=true&row=1&column=6"/>

</p>

---

# 👀 Visitantes

<p align="center">

<img src="https://komarev.com/ghpvc/?username=walterprestes007-bot&label=Visualizações&color=A855F7&style=flat"/>

</p>

---

# 📈 Perfil

<p align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=walterprestes007-bot&theme=github_dark"/>

</p>
---

# 💡 Curiosidades

```java
while (alive) {

    aprender();

    praticar();

    melhorar();

}
```

---

# 💬 Frase

<p align="center">

> **"Transformando ideias em código e desafios em oportunidades."** 🚀

</p>

---

<div align="center">

## ⭐ Obrigado por visitar meu perfil!

Se gostou dos meus projetos, não esqueça de deixar uma ⭐ nos repositórios.

🚀 Sempre aberto para aprender, colaborar e desenvolver novas ideias.

</div>

---

<p align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=8A2BE2&height=150&section=footer"/>

</p>
